...
//	@RequestMapping(value = "test1", method = RequestMethod.GET)
	...
	public String test2(
		...
		// -> HTML에서 얻어오는 모든 갓은 기본적으로 String형태이나
		//		 자동으로 형변환/ 파싱을 수행해준다!!!
...
public String test4(
	 ...
	 * * required :  필수
	 * -> 기본 값이 true 
	 * -> true가 설정된 경우 == 꼭 제출되어야 하는 파라미터
	 * 		--> 만약 제출되지 않을 경우
	 *  	HTTP 상태코드 400 -> 400 Bad Request 에러 발생 
	 *  
	 *  @ checkbox, radio : 체크되지 않으면 제출 X -> 400 에러 발생
	 *  
	 *  @ 직접 쿼리스트링을 작성하는데
	 *     	key-value가 누락된 경우 -> 400 에러
	 *     
	 *     * required = false일 때, 값이 제출되지 않으면 -> null
	 *     
	 *     * defaultValue :
